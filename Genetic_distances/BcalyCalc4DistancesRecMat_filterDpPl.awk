...
BEGIN {print "-----Begin BcalyCalcDistance_filterPl.awk-----";
	...
	tableFileName3 = outputFileName ".HomoDistanceTable.Dp" thresholdDp "Pl" thresholdPl ".tbl";
	tableFileName4 = outputFileName ".HeteroDistanceTable.Dp" thresholdDp "Pl" thresholdPl ".tbl";
...
{	
	...
	if(FNR==1){
		print "thresholdPl\tSample1\tSample2\tnumSiteCompared\tdistance1\tdistance2\tdistance3\tdistance4\tnumRefHet\tnumAltHet\tnumHetHet\tnumRefAlt\tnumRefRef\tnumAltAlt" > outFileName ;
	}
	...
	if($0 ~ /\#/){
		...
		for(i = 10; i <= NF; i++){
			...
			for(j = 10; j <=NF; j++){
				...
				if($i~/^0\/0/ || $i~/^0\/1/ || $i~/^1\/0/ || $i~/^1\/1/ || $i~/^0\|0/ || $i~/^0\|1/ || $i~/^1\|0/ || $i~/^1\|1/){
				...
				if($j~/^0\/0/ || $j~/^0\/1/ || $j~/^1\/0/ || $j~/^1\/1/ || $j~/^0\|0/ || $j~/^0\|1/ || $j~/^1\|0/ || $j~/^1\|1/ ){
...
END {
	...
	for(i = 1; i <=numSample; i++){
		for(j = 1; j <= numSample; j++){
			...
			if(numSiteCompared[i,j] > 0){
				...
				distance3[i,j] /= numSiteCompared[i,j];
				distance4[i,j] /= numSiteCompared[i,j];
			}
	...
	for(i = 1; i <=numSample; i++){
		for(j = 1; j <= numSample; j++){
			print thresholdPl "\t" sampleName[i] "\t" sampleName[j] "\t" numSiteCompared[i,j] "\t" distance1[i,j] "\t" distance2[i,j] "\t" distance3[i,j] "\t" distance4[i,j] "\t" numRefHet[i,j] "\t" numAltHet[i,j] "\t" numHetHet[i,j] "\t" numRefAlt[i,j] "\t" numRefRef[i,j] "\t" numAltAlt[i,j] > outFileName;
			...
			distance3Table[i,j]=distance3[i,j]
			distance3Table[j,i]=distance3[i,j]
			distance4Table[i,j]=distance4[i,j]
			distance4Table[j,i]=distance4[i,j]			
		}
	}
	...
	printf "Distance3\t" > tableFileName3
	printf "Distance4\t" > tableFileName4

	for(i = 1; i <=numSample; i++){
		printf sampleName[i] > tableFileName1
		printf sampleName[i] > tableFileName2
		printf sampleName[i] > tableFileName3
		printf sampleName[i] > tableFileName4
		if(i<numSample){
			printf "\t" > tableFileName1
			printf "\t" > tableFileName2
			printf "\t" > tableFileName3
			printf "\t" > tableFileName4
		}
		else{
			printf "\n" > tableFileName1
			printf "\n" > tableFileName2
	...
	for(i = 1; i <=numSample; i++){
		...
		for(j = 1; j <= numSample; j++){
			...
			printf distance3Table[i,j] > tableFileName3
			printf distance4Table[i,j] > tableFileName4
