...
both.distance1.pl30 <- read.table("distance_PVE_both_PGAbfp_HC_50miss_indmiss_qualOdp_fqbd_noindels.distance1Table.DpPl30.tbl", 
                                             header = T, check.names=FALSE, row.names = 1)
# just checking file sctructure and else meta.data
str(both.distance1.pl30)
head(both.distance1.pl30)
range(both.distance1.pl30)
dim(both.distance1.pl30)

# minDp30maxDp250Pl30Ind60[upper.tri(minDp30maxDp250Pl30Ind60)] <- "NA" 
# now I do convert it into a distance object
D1_both.distance1.pl30 <- as.dist(both.distance1.pl30)
D1_both.distance1.pl30 %>% head
library(cluster)
D1_both.distance1.pl30 %>% sizeDiss()
# Calculate the dendrogram
dend1_both.dist1 <- as.dendrogram(hclust(D1_both.distance1.pl30))
dend1_both.dist1 <- set(dend1_both.dist1, "labels_cex", 1)
...
keysN_ordered <- keysN[match(labels(dend1_both.dist1), row.names(keysN)), ] # notice that now, I am ordering the fucking keys by the dend using match
cbind.data.frame(labels(dend1_both.dist1), row.names(keysN_ordered)) # and it is in the same order wohoo...
...
labels_colors(dend1_both.dist1) <- colorCodesBasin #[order.dendrogram(dend1_new)]

# plot the dendro
png(file="dend1_both_dist1_color.png", width=1200, height=2000)
plot(dend1_both.dist1, horiz = TRUE, main = "Dendrogram for E. both samples", cex = 1)
